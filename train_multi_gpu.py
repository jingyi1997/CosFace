...
def main(args):
    ...
    #print('nrof_classes: ',nrof_classes)#10575
    image_list, label_list = utils.get_image_paths_and_labels(train_set)
    image_list = np.array(image_list)#452960
    #image_list = image_list.astype("str")
    label_list = np.array(label_list,dtype=np.int32)
    #print(image_list.dtype)
    #print(image_list[1126:1186])
    #print(label_list[1126:1186])
    #print(label_list.dtype)
    dataset_size = len(image_list)
    single_batch_size = args.people_per_batch*args.images_per_person#150 = 30*5
    indices = range(dataset_size)
    np.random.shuffle(indices)
    print('dataset_size:',dataset_size)
    print('images per person:',args.images_per_person)
	...
	#print(sample_labels.dtype)
        ...
        return np.array(sample_paths,dtype='|S44'),np.array(sample_labels,dtype=np.int32)
    ...
    with tf.Graph().as_default():
        ...
        with tf.device("/cpu:0"):
            ...
            softmax_dataset = softmax_dataset.map(lambda x: tuple(tf.py_func(_sample_people_softmax,[x],[tf.string,tf.int32])))
        ...
        print(sess.run(softmax_iterator.initializer))
